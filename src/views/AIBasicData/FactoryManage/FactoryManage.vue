  ...
  <div>
      ...
      <el-row :span="20">
        <el-col :span="4" style="margin-right: 20px">
          <el-form-item label="">
            ...
            <el-input v-model="SearchFrom.keyword" :placeholder="$t('lang.ImesData.plzFactory')" clearable />
          </el-form-item>
        </el-col>
        <el-col :span="4">
    ...
    <div class="btns">
      <el-button type="primary" @click="editData('', 1)" :icon="Plus">{{
        $t('lang.ImesData.addFactory')
      }}</el-button>
    </div>
    <el-table :data="datas.data" border stripe style="width: 100%; margin-top: 20px">
      <el-table-column prop="factoryCode" :label="$t('lang.ImesData.FactoryBs')" show-overflow-tooltip />
      <el-table-column prop="factoryName" :label="$t('lang.ImesData.FactoryName')" show-overflow-tooltip />
    ...
    <FactoryManageEdit v-if="datas.showEdit" @close="close" :editData="datas.editData" :isShow="datas.show"
      @success="searchData"></FactoryManageEdit>
  </div>
</template>
...
import FactoryManageEdit from './FactoryManageEdit.vue'
...
const DeleteBtn = (data: any) => {
  ...
  useConfirm(
    ...
    `-${data.factoryName}[${data.factoryCode}]` +
    $t('lang.common.delTipTwo')
  )
}
const deleteData = (params: any) => {
  proxy.$api.FactoryManage.FactoryDel(params).then((res: any) => {
    datas.loading = false
    useCallback(res)
    searchData()
  })
